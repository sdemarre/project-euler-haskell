...
isPalindrome xs = let isPalindrome_rec :: (Eq a, Integral b) => [a] -> b -> [a] -> Bool
                      ...
                      isPalindrome_rec (x:xs) 0 (y:ys) = and [x == y, isPalindrome_rec xs 0 ys]
...
digitsToNumber xs = let digitsToNumber_rec :: (Integral a) => a -> [a] -> a
                        digitsToNumber_rec n [] = n
                        digitsToNumber_rec n (x:xs) = digitsToNumber_rec (10*n+x) xs
                    in digitsToNumber_rec 0 xs
...
problem_10_n n = 2 + (foldr1 (+) (takeWhile (\x -> x < n) Primes.primes))
...
problem_12 = problem_12_n 500

problem_13_data = [37107287533902102798797998220837590246510135740250,
...
problem_13 = digitsToNumber $ take 10 $ numberToDigits $ sum problem_13_data
...
problem_14_fails limit = foldl1 (\ l@(_:ll) r@(_:lr) -> if ll > lr then l else r)
                       $ map (\ n -> (n: [length $ collatz n])) [1..limit]

incrementalCollatzCount n counts
  | Data.Maybe.isJust ( Map.lookup n counts ) = counts
  | otherwise = let steps = 1 + Data.Maybe.fromJust (Map.lookup (collatzStep n) $ incrementalCollatzCount (collatzStep n) counts)
                in Map.insert n steps counts
