...
problem67 = bestPathInTriangle problem67Data


problem18 = bestPathInTriangle problem18Data

bestPathInTriangle problemData = maximum $ foldl1 computeNewBestLine problemData
  where computeNewBestLine currentBest newLine = computeNewBestLine_rec (0:currentBest) newLine
          where computeNewBestLine_rec [x] [y] = [x+y]
                computeNewBestLine_rec (x:xs) (y:ys) = (max (x+y) (y+(head xs))):(computeNewBestLine_rec xs ys)
